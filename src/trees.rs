...
impl Tree {
    ...
    fn from(size: usize, opt_left: Option<Rc<RefCell<Tree>>>, opt_up: Option<Rc<RefCell<Tree>>>) -> Tree {
        let max_left = Tree::get_max(opt_left, |left| left.max_left);
        let max_up = Tree::get_max(opt_up, |up| up.max_up);
        Tree { size: size, max_left: max_left, max_up: max_up, max_right: None, max_down: None }
    }

    fn set_right_and_down(&mut self, opt_right: Option<Rc<RefCell<Tree>>>, opt_down: Option<Rc<RefCell<Tree>>>) {
        self.max_right = Tree::get_max(opt_right, |right| right.max_right);
        self.max_down = Tree::get_max(opt_down, |down| down.max_down);
    }

    fn get_max<F>(opt_delimiting_tree: Option<Rc<RefCell<Tree>>>, get_max_boundary: F) -> Option<usize> where F: Fn(Ref<Tree>) -> Option<usize> {
    ...
    if let Ok(lines) = lines(tree_house_filename) {
        for maybe_line in lines {
            if let Ok(line) = maybe_line {
                ...
                for tree_size in line.chars()
                        .map(|c| c.to_string())
                        .map(|c| usize::from_str_radix(&c, 10).unwrap()) {
                    let opt_left = if col_idx > 0  { Some(current_line[col_idx - 1].to_owned()) } else { None };
                    let opt_up = if tree_house.len() > 0  { Some(tree_house[tree_house.len() - 1][col_idx].to_owned()) } else { None };
                    current_line.push(Rc::new(RefCell::new(Tree::from(tree_size, opt_left, opt_up))));
                    col_idx += 1;
                }
    ...
    for line_idx in (0..=max_idx).rev() {
        ...
        for col_idx in (0..=max_idx).rev() {
            if let Ok(mut tree) = current_line[col_idx].try_borrow_mut() {
                let opt_right = if col_idx < max_idx  { Some(current_line[col_idx + 1].to_owned()) } else { None };
                let opt_down = if line_idx < max_idx  { Some(tree_house[line_idx + 1][col_idx].to_owned()) } else { None };
