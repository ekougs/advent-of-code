...
pub fn prioritize_for_diff_in_rumsack(rucksacks_filename: &str) -> u32 {
    ...
    if let Ok(lines) = lines(rucksacks_filename) {
        ...
        for maybe_line in lines {
            if let Ok(line) = maybe_line {
                ...
                for rucksack_item in line_char_set {
                    lines_group_hashmap
                        .insert(rucksack_item, lines_group_hashmap[&rucksack_item] - 1);
                }
                if nb_lines == group_count {
                    let common_rucksack_item_entry = lines_group_hashmap
                        .iter()
                        .filter(|entry| *(entry.1) == 0)
                        .next();
                    match common_rucksack_item_entry {
                        Some((common_rucksack_item, _)) => {
                            priorities_sum += priority(*common_rucksack_item)
                        }
